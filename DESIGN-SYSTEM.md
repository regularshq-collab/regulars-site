...
Contrast, measured in-browser across all nine pages: paper/navy 14.8:1 ·
steel/paper 6.2:1 · **steel-lift/navy 4.77:1** · steel-lift/navy-deep 5.48:1 ·
navy/amber 8.1:1. Banned: amber text on paper; steel text on navy (2.0:1 — rules only).

Dark grounds re-point **`--steel` to `--steel-lift`** automatically, exactly as they do
`--rule-color`. So `color:var(--steel)` is now safe on any ground and components need no
hand-written `.on-dark` variant to stay legible. Do not undo this: roughly thirty rules set
`color:var(--steel)` and only some had overrides, which is how table captions and the
calculator's own labels ended up at 2.3:1 on navy.
...
`--steel #2C5F7C` · `--steel-lift #6693B1` · `--amber #E8A33D` · `--amber-deep #C9862A` ·
...
```html
<span class="stick">
  <i class="sym">$</i><i>2</i><i>9</i><i>7</i><i class="unit">/mo</i>
</span>
...
<div class="plate">
  ...
  <dl class="plate__body">
    ...
    <div class="plate__row"><dt>Service</dt><dd><em>$297</em> / month</dd></div>
...
<dl class="spec">
  ...
  <div class="spec-row"><dt>Service</dt><span class="leader"></span><dd>$297/mo</dd></div>
</dl>
```
...
Pricing copy is fixed and must be stated exactly: **$500 one-time setup + $297/month, cancel
